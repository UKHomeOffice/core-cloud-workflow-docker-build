...
Contributing to `core-cloud-workflow-docker-build`
...
* `action.yaml` — composite GitHub Action for running docker build

* `tests/**` — intentionally invalid docker test fixtures
...
These directories contain intentionally broken docker code designed solely for:

* docker build regression tests

* docker build behaviour validation
...
If you add real docker code here, it will be ignored by SAST and may mislead users.
